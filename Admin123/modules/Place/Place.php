...
class Place extends Module
{
    public static function form($args)
    {
        ...
        if (Tools::getValue('id')){
            ...
            $record  = (myPlace::getById($list_id) !== null)?myPlace::getById($list_id)[0]:array();
        }
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $list_id = myPlace::save();
            $record  = myPlace::getById($list_id)[0];
            $btnType = 'Edit';
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            */
            ...
            $_POST["date_upd"] = $currentDate;
            myPlace::updateById(Tools::getValue('id'));
            $record = myPlace::getById(Tools::getValue('id'))[0];
            //Tools::refresh();
        }
        ...
        /* Form Title */
        $ret.= '    <div class="row">';
        $ret.= '       <span class="frm-title">'.RTTCore::l('Place','admin').'</span>';
        $ret.= '    </div>';
        /* message Actions */
        if ($messages)
        {
        ...
        $ret.= '           <div class="col-1">';
        $ret.= '               <div class="groupform">';
        $ret.= '                   <label>'.RTTCore::l('Capacity').'</label>';
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="capacity" type="text" value="'.(!empty($record['capacity'])?$record["capacity"]:"").'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
    ...
    public static function hookPlace($args)
    {
