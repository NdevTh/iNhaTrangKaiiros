...
class ProductSale extends Module
{
    public static function form()
    {
        ...
        $newCode = 'ART00001';
        //echo RTTCore::l("Test");
		//$ret .= myArticle::getByWhere('id_categorie='._PRODUCT_SALE)[0]['code'];
        if (myArticle::getByWhere('id_categorie='._PRODUCT_SALE_) && Tools::getValue("id") == 0 )
        {
			$indRow = (count(myArticle::getByWhere('id_categorie='._PRODUCT_SALE_)) - 1);
			$number = ((int)str_replace("ART","",myArticle::getByWhere('id_categorie='._PRODUCT_SALE_)[$indRow]['code']) + 1);
            $newCode = !empty(myArticle::getByWhere('id_categorie='._PRODUCT_SALE_))?('ART'.RTTCore::addZeroBeforeNum($number,5)):$newCode;
        }
        ...
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $_POST["id_categorie"] = _PRODUCT_SALE_;
        ...
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="installed_date" type="date" value="'.((!empty($record) && !empty($record['installed_date']))?$record["installed_date"]:date('Y-m-d')).'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
    ...
    public static function hookProductSale($args)
    {
