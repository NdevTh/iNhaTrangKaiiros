...
{   
    ...
    {
        global $AdminUser, $typeEmployee, $statusUser, $gender;
        myJob::init();
        $record      = array();
        $btnType     = 'Add';
		$messages    = array();
        ...
        $list_id     = 0;
		$ret         = '';
        $_POST['system_role'] = _EMP_SYSTEM_ROLE_;
        ...
        if (Tools::isSubmit('btnSubmitAdd')){
            //$ret.= '<div class="msg">';
            $messages[$btnType]= '   Action "'.$btnType.'" was successful';
            //$ret.= '</div>';
            ...
            {
                $_POST['password'] = Tools::encrypt($_POST['password']);
            }
			...
			$xmlFields = array('id','title','id_user','id_lang','bg_color','nav');
			$_POST['id'] = $list_id;
			$_POST['title'] = 'user' . $list_id;
			$_POST['id_user'] = $list_id;
			$_POST['id_lang'] = 1;
			$_POST['bg_color'] = "bg-orange";
			$_POST['nav'] = "close";
			
			BundlesXMLFile::writeFile('themes',$xmlFields,"","../Bundles/xml");
        }
        ...
        {
            $recordJob = !empty(myJob::getById($record['id_job']))?myJob::getById($record['id_job'])[0]:array();
        }
        $recordCompa = array();
        if (isset($record['id_company']))
        {
            $recordCompa = !empty(myCompany::getById($record['id_company']))?myCompany::getById($record['id_company'])[0]:array();
        }
        /* message Actions */
        if ($messages){
            $ret.= '<div class="msg">';
        ...
        $ret.= '                   <input name="type" type="hidden" value="'.(!empty($record['type'])?$record['type']:0).'"/>';
        $ret.= '                   <input name="type_title" type="text" value="'.(!empty($record)?(RTTCore::l($typeEmployee[$record['type']],'admin')):(RTTCore::l($typeEmployee[Tools::getValue('tp')],'admin'))).'"/>';$ret.= '               </div>';
        $ret.= '           </div>';
        ...
        $ret.= '                   <input name="status_title" type="text" value="'.(!empty($record['status'])?RTTCore::l($statusUser[$record["status"]]):RTTCore::l($statusUser[$record["status"]])).'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
		...
		$ret.= '           <div class="col-1">';
        $ret.= '               <div class="groupform">';
        $ret.= '                   <label>'.RTTCore::l('Signature Image').'</label>';
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="img_signature" type="text" value="'.(!empty($record['img_signature'])?$record["img_signature"]:"").'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
        ...
        $ret.= '           <div class="col-1">';
        $ret.= '               <div class="groupform">';
        $ret.= '                   <label>'.RTTCore::l('Gender').'</label>';
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <select name="id_gender">';
		foreach($gender as $k => $val)
		{
			if ($k == $record['id_gender'])
			{
				$ret.= '                   <option value="'.$k.'" selected>'.RTTCore::l($val).'</option>';
			}else{
				$ret.= '                   <option value="'.$k.'" >'.RTTCore::l($val).'</option>';
			}
    ...
    public static function hookEmployee($args)
    {
        ...
        myCompany::init();
        myJob::init();
		if (Tools::getValue("act") == 'del' && Tools::getValue("id") != 0)
		{
			myUser::deleteByWhere("id_user=".Tools::getValue("id"));
		}
        $records = myUser::getByWhere('system_role=' . _EMP_SYSTEM_ROLE_ . ' OR system_role=' . _EMP_SYSTEM_ROLE_);
        ...
        $ret.= '  <div class="row tools-title">';
        $ret.= '     <span class="tbl-title">'.RTTCore::l('Employees','admin').'</span>';
