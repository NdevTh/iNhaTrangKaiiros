...
class ExQuotation extends Module
{    
    public static function form($args)
    {
        ...
        $newCode = 'QX00001';
        //echo RTTCore::l("Test");
        if (myQuotationOrder::getByWhere("id_categorie="._EXTERNAL_QUOTATION_)){
            $indRow = (count(myQuotationOrder::getByWhere("id_categorie="._EXTERNAL_QUOTATION_)) - 1);
            $number = ((int)str_replace("QX","",myQuotationOrder::getByWhere("id_categorie="._EXTERNAL_QUOTATION_)[$indRow]['code']) + 1);
            $newCode = !empty(myQuotationOrder::getLastRecord())?('QX'.RTTCore::addZeroBeforeNum($number,5)):$newCode;
        }
        ...
        if (Tools::isSubmit('btnSubmitAdd')){
			...
			$_POST["id_categorie"] = _EXTERNAL_QUOTATION_;
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            $record = !empty(myQuotationOrder::getById(Tools::getValue('id'))[0])?myQuotationOrder::getById(Tools::getValue('id'))[0]:array();
            //Tools::refresh();
        }
        
        $recordWorkOrder = array();
        if (!empty($record["id_workorder"]))
        {
            $recordWorkOrder = !empty(myWorkOrder::getById($record['id_workorder']))?myWorkOrder::getById($record['id_workorder'])[0]:array();
            $id_workorder = $record["id_workorder"];
        }
        $recordOwner = array();
        if (!empty($record['id_created_by']))
        {
            $recordOwner = !empty(myUser::getById($record['id_created_by']))?myUser::getById($record['id_created_by'])[0]:array();
        }
        
        $recordOrder = array();
        if (!empty($record['id_order_by']))
        {
            $recordOrder = isset(myUser::getById($record['id_order_by'])[0])?myUser::getById($record['id_order_by'])[0]:array();
            $id_order_by = $record["id_order_by"];
        }
            ...
            if(isset($_POST['arrid_quotationdetail'])){
                ...
                foreach($_POST['arrid_quotationdetail'] as $id_qd_report)
                {
                    ...
                    if ($id_qd_report == 0){
                        ...
                        $_POST['refference']                 = $_POST['arrrefference'][$indRow];
        ...
        $ret.= self::reportPreview($list_id,$id_workorder,$id_order_by);
        $ret.= '       </div>';
        /*<!-- End The Modal -->*/
    ...
    public static function reportPreview($idQuotation,$idWorkOrder,$idCustomer=0,$idOwner=1)
    {
        ...
        if ($record)
        {
            ...
            $intervenant = !empty(myUser::getByWhere("id_user = '".$record["id_created_by"]."'"))?myUser::getByWhere("id_user = ".$record["id_created_by"]."")[0]:array();
            $ownerCompany = !empty($intervenant)?myCompany::getById($intervenant["id_company"])[0]:array();
            $ownerAddress = !empty($ownerCompany)?myAddress::getByWhere("id_company = '".$ownerCompany["id_company"]."'")[0]:array();
            
            $customer = !empty(myUser::getByWhere("id_user = '".$record["id_order_by"]."'"))?myUser::getByWhere("id_user = ".$record["id_order_by"]."")[0]:array();
            $cusCompany = !empty($customer)?myCompany::getById($customer["id_company"])[0]:array();
            $cusAddress = !empty($cusCompany)?myAddress::getByWhere("id_company = '".$cusCompany["id_company"]."'")[0]:array();
        ...
        //$ret.= '            Test';
        $ret.= '            <div class="row tools-title">';
        $ret.= '               <span class="tbl-title">	'.RTTCore::l('Quotation').'</span>';
        ...
        $ret.= '                     	'.RTTCore::l('Identification Of Document');
        $ret.= '                  </th>';
        $ret.= '                  <th class="txt-center">';
        $ret.= '                      	'.RTTCore::l('Revision');
        $ret.= '                  </th>';
        $ret.= '                  <th class="txt-center">';
        $ret.= '                      	'.RTTCore::l('Application Date');
        $ret.= '                  </th>';
        $ret.= '                  <th class="txt-center">';
        $ret.= '                      Page	';
        $ret.= '                  </th>';
        ...
        $ret.= '                      '.RTTCore::l('Quotation');
        $ret.= '                  </th>';
        $ret.= '                  <th colspan="2">';
        $ret.= '                      '.RTTCore::l('Document Info');
        $ret.= '                  </th>';
        $ret.= '                 <tr>';
        $ret.= '                  <th class="txt-left">';
        $ret.= '                      '.RTTCore::l('Quotation Number');
        $ret.= '                  </th>';
        $ret.= '                  <td colspan="2">';
        $ret.= '                      	'. $record['code'];
        $ret.= '                  </td>';
        ...
        $ret.= '                 <tr>';
        $ret.= '                  <th colspan="2">';
        $ret.= '                      '.RTTCore::l('Company Infomation');
        $ret.= '                  </th>';
        $ret.= '                  <th colspan="2">';
        $ret.= '                      '.RTTCore::l('Customer Infomation');
        $ret.= '                  </th>';
        $ret.= '                 </tr>';
        
        $ret.= '                 <tr>';
        $ret.= '                  <td class="tbl-field">';
        $ret.= '                      '.RTTCore::l('Company');
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-value">';
        $ret.= '                      '. $ownerCompany['company'];
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-field">';
        $ret.= '                      '.RTTCore::l('Company');
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-value">';
        $ret.= '                      '. $cusCompany['company'];
        $ret.= '                  </td>';
        $ret.= '                 </tr>';
        
        $ret.= '                 <tr>';
        $ret.= '                  <td class="tbl-field">';
        $ret.= '                      '.RTTCore::l('Address');
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-value">';
        $ret.= '                      '. $ownerAddress['address'];
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-field">';
        $ret.= '                      '.RTTCore::l('Address');
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-value">';
        $ret.= '                      '. $cusAddress['address'];
        $ret.= '                  </td>';
        $ret.= '                 </tr>';
        
        $ret.= '                 <tr>';
        $ret.= '                  <td class="tbl-field">';
        $ret.= '                      '.RTTCore::l('Postal Code');
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-value">';
        $ret.= '                      '. $ownerAddress['postal_code'];
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-field">';
        $ret.= '                      '.RTTCore::l('Postal Code');
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-value">';
        $ret.= '                      '. $cusAddress['postal_code'];
        $ret.= '                  </td>';
        $ret.= '                 </tr>';
        ...
        $ret.= '                 <tr>';
        $ret.= '                  <td class="tbl-field">';
        $ret.= '                      '.RTTCore::l('Created Date');
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-value">';
        $ret.= '                      '. $record['date_add'];
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-field">';
        $ret.= '                      '.RTTCore::l('Expired Date');
        $ret.= '                  </td>';
        $ret.= '                  <td class="tbl-value">';
        $ret.= '                      '. $record['date_estimated'];
        $ret.= '                  </td>';
        $ret.= '                 </tr>';
        ...
        $ret.= '                  </th>';
        $ret.= '                  <th>';
        $ret.= '                      '.RTTCore::l('Description');
        $ret.= '                  </th>';
        $ret.= '                  <th>';
        $ret.= '                      '.RTTCore::l('Quantity');
        $ret.= '                  </th>';
        $ret.= '                  <th>';
        $ret.= '                      '.RTTCore::l('Unit Price');
        $ret.= '                  </th>';
        $ret.= '                  <th>';
        $ret.= '                      '.RTTCore::l('Total HT');
        $ret.= '                  </th>';
        $ret.= '                  <th>';
        $ret.= '                      '.RTTCore::l('Discount(%)');
        $ret.= '                  </th>';
        $ret.= '                  <th>';
        $ret.= '                      '.RTTCore::l('Discount');
        $ret.= '                  </th>';
        ...
        if ($recordsQuotationDetail)
        {
            foreach($recordsQuotationDetail as $recordQuotationDetail)
            {
                ...
                $ret.= '                 <tr>';
                $ret.= '                    <td>';
                $ret.= $recordQuotationDetail['refference'];
                $ret.= '                    </td>';
                $ret.= '                    <td>';
    ...
    public static function modalOrder($idQuotation)
    {
        ...
        //$recordsWorkOrder = array();
        //if ($record){
        $recordsUserOrder = !empty(myUser::getByWhere("system_role="._CUS_SYSTEM_ROLE_))?myUser::getByWhere("system_role="._CUS_SYSTEM_ROLE_):array();
        //}
        //var_dump($recordsEquipment);
        $ret.= '   <div class="row tools-title">';
    ...
    public static function modalOwner($idQuotation)
    {
        ...
        //$recordsWorkOrder = array();
        //if ($record){
            $recordsUserOrder = myUser::getByWhere("system_role="._ADM_SYSTEM_ROLE_." OR system_role="._EMP_SYSTEM_ROLE_);
        //}
        //var_dump($recordsEquipment);
        $ret.= '   <div class="row tools-title">';
        $ret.= '     <span class="tbl-title">'.RTTCore::l("Order By").'</span>';
        ...
        */
        ...
        if ($recordsUserOrder)
        {
            ...
            foreach($recordsUserOrder as $recordUserOrder)
            {
                $ret.= '  <tr>';
                $indRow++;
                $ret.= '        <td class="txt-center"><input  onchange="changeChecked(\'userowner\',this.id)" id="userowner'.$indRow.'" type="checkbox" data-listId="'.(!empty($recordUserOrder['id_user'])?$recordUserOrder['id_user']:$indRow).'" data-img="'.(!empty($recordUserOrder['img_name'])?$recordUserOrder['img_name']:'blank.jpg').'"  value="'.(!empty($recordUserOrder['full_name'])?$recordUserOrder['full_name']:(!($recordUserOrder['first_name'])?$recordUserOrder['first_name']:$indRow)).'"/></td>';
                foreach($fieldsAsset as $field => $th)
                {
                    if ($field == 'id_company')
                    {
    ...
    public static function hookExQuotation($args)
    {
        ...
        $records = myQuotationOrder::getByWhere("id_categorie="._EXTERNAL_QUOTATION_);
        $ret.= '<div class="row tools-title">';
        $ret.= '   <div class="col-2 txt-left">';
        //$ret.= '     <a class="btn" href="index.php?t='.Tools::getValue('t').'&pt='.Tools::getValue('pt').'&token='.Tools::getValue('token').'"><i class="bx bx-arrow-to-left"></i>'.RTTCore::l("Return","Admin").'</a>';
        $ret.= '   </div>';
