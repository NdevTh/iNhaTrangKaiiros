...
class Hero extends Module
{
    public static function form($args)
    {
        ...
        if (Tools::getValue('id')){
            ...
            $record  = (myHero::getById($list_id) !== null)?myHero::getById($list_id)[0]:array();
        }
        ...
        /* Form Title */
        $ret.= '    <div class="row">';
        $ret.= '       <span class="frm-title">'.RTTCore::l('Hero Page','admin').'</span>';
        $ret.= '    </div>';
        /* message Actions */
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $list_id = myHero::save();
            $record  = myHero::getById($list_id)[0];
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            $_POST["date_upd"] = $currentDate;
            myHero::updateById(Tools::getValue('id'));
            $record = myHero::getById(Tools::getValue('id'))[0];
            //Tools::refresh();
        }
        ...
        $ret.= '           <div class="col-1">';
        $ret.= '               <div class="groupform">';
        $ret.= '                   <label>'.RTTCore::l('Sub Title').'</label>';
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="subtitle" type="text" value="'.(!empty($record['subtitle'])?$record["subtitle"]:"").'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
        ...
        $ret.= '           <div class="col-1">';
        $ret.= '               <div class="groupform">';
        $ret.= '                   <label>'.RTTCore::l('Background Image').'</label>';
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="img_banner_bg" type="text" value="'.(!empty($record['img_banner_bg'])?$record["img_banner_bg"]:"").'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
        ...
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="img_banner" type="text" value="'.(!empty($record['img_banner'])?$record["img_banner"]:"").'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
    ...
    public static function hookHero($args)
    {
        global $actions,$status,$actives;
        $records = array();
        $table = "hero";
        myHero::init();
        $records = myHero::getRecords();
        
        $fieldsDisplay = array(
            'id_lang'      => RTTCore::l('Language'),
            'code'         => RTTCore::l('Code'),
            'subtitle'     => RTTCore::l('Sub Title'),
            'title'        => RTTCore::l('Title'),
            'description'  => RTTCore::l('Description'),
            'img_banner_bg'=> RTTCore::l('Background Image'),
            'img_banner'   => RTTCore::l('Image'),
