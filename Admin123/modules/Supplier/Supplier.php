...
class Supplier extends Module
{
    public static function form($args)
    {
        ...
        */
        $newCode = 'F00001';
        if (myCompany::getByWhere('system_role='._SUP_SYSTEM_ROLE_))
        {
            $indRow = (count(myCompany::getByWhere('system_role='._SUP_SYSTEM_ROLE_)) - 1);
            $number = ((int)str_replace("F","",myCompany::getByWhere('system_role='._SUP_SYSTEM_ROLE_)[$indRow]['code']) + 1);
            $newCode = !empty(myCompany::getByWhere('system_role='._SUP_SYSTEM_ROLE_))?('F'.RTTCore::addZeroBeforeNum($number,5)):$newCode;
        }
        ...
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $recordDetail = !empty(myAddress::getByWhere("`id_company`=".$list_id))?myAddress::getByWhere("`id_company`=".$list_id)[0]:array();
        ...
        $ret.= '                   <input name="type" type="hidden" value="'.(!empty($record)?$record["type"]:(!empty(Tools::getValue('tp'))?Tools::getValue('tp'):0)).'"/>';
        $ret.= '                   <input name="type" type="text" value="'.(!empty($record)?RTTCore::l($typeEmployee[$record["type"]]):RTTCore::l($typeEmployee[Tools::getValue('tp')],'admin')).'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
        ...
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="status" type="hidden" value="'.(!empty($record)?$record["status"]:0).'"/>';
        $ret.= '                   <input name="status_title" type="text" value="'.(!empty($record)?RTTCore::l($statusUser[$record["status"]]):RTTCore::l($statusUser[0])).'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
    ...
    public static function openingHoursTab($idCompany)
    {
        ...
        $record  = !empty(myCompany::getById($idCompany))?myCompany::getById($idCompany)[0]:array();
        $ret = '';
    ...
    public static function modalLang()
    {
        ...
        if ($records)
        {
            ...
            foreach($records as $record)
            {
                ...
                $ret.= '        <td class="txt-center">';
                //$ret.= '           <a href="index.php?t='.Tools::getValue('t').'&act=form&id='.$record["id"].'&token='.Tools::getValue('token').'"><img class="tbl-ico16" src="../Bundles/images/advancetools/10629723.png"/></a>';
                //$ret.= '           <a  onclick="return confirm(\'Vous êtes sûr?\');" href="index.php?t='.Tools::getValue('t').'&act=del&id='.$record["id"].'&token='.Tools::getValue('token').'"><img class="tbl-ico16" src="../Bundles/images/advancetools/10871842.png"/></a>';
                $ret.= '        </td>';
                $ret.= '      </tr>';
            }
        }
    ...
    public static function hookSupplier($args)
    {
        ...
        $fieldsDisplay = array(
            ...
            'principal_contact'   => RTTCore::l('Principal Contact'),
            'telephone_principal' => RTTCore::l('First Phone Number'),
            'telephone_seconde'   => RTTCore::l('Second Phone Number'),
        );
        myUser::init();
        myCompany::init();
        $records = myCompany::getByWhere('`system_role` = '._SUP_SYSTEM_ROLE_);
        ...
        if ($records)
        {
            ...
            foreach($records as $record)
            {
                ...
                foreach($fieldsDisplay as $field => $th)
                {
                    ...
                    }else if (array_key_exists('status',$record) && $field == 'status')
                    {
