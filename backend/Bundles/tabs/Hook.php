...
class Hook extends AdminTab
{
    public static function form()
    {
        ...
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $ret.= '</div>';
            //var_dump($_POST);
            $id = XMLFile::writeFile('hooks',array(),"","xml");
            $whereClause = '[@id="'.$id.'"]';
            $query = '//hooks/hook';
            $hooks = XMLFile::getXMLByWhere('hooks',$query,$whereClause,'xml');
            $record = $hooks[0];
            
            $whereClause = '[@id="'.$record["id_module"].'"]';
            $query = '//modules/module';
            $modules = XMLFile::getXMLByWhere('hooks',$query,$whereClause,'xml');
            $module = $modules[0];
            ...
            $content .= '} ?>';
            echo $content;
            File::putContent($filename,$content,"../modules/".$filename);
        }
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            XMLFile::update('hooks',$whereClause,$updateFields,$query,"xml");
        }
        ...
        $ret.= '<form action="core.php?'.Tools::newUrl('t','hook').'" method="POST" class="form">';
        $ret.= '  <fieldset class="edge-rond">';
        $ret.= '    <legend class="">Hook Form</legend>';
        ...
        $modules = XMLFile::readFile('modules','','xml');
        if (!empty($modules)){
    ...
    public static function viewHook()
    {
        $ret = '';
        //$ret.= 'view Hook';
        if (Tools::getValue('act') == 'del'){
            ...
            $ret.= '</div>';
            XMLFile::deleteById('hooks',Tools::getValue('id'),'xml');
        }
        if (!file_exists('xml/hooks.xml'))
        {
            $ret.= 'Initialization Hook file...';
            Core::initHook();
        }
        $records = XMLFile::readFile('hooks','','xml');
        //var_dump($records);
        if (!empty($records))
        {
            $fields = XMLFile::getFields('hooks','xml');
            $ret.= '<div class="row">';
