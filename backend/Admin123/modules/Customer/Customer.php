...
class Customer extends Module
{
    public static function form($args)
    {
        ...
        $list_id     = 0;
        $_POST['system_role'] = _CUS_SYSTEM_ROLE_;
        ...
        /* Form Title */
        $ret.= '    <div class="row">';
        $ret.= '       <span class="frm-title">'.RTTCore::l('Customer').'</span>';
        $ret.= '    </div>';
        /* message Actions */
        ...
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="full_name" type="text" value="'.(!empty($record['full_name'])?$record["full_name"]:"").'" required/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
    ...
    public static function hookCustomer($args)
    {
        ...
        myCompany::init();
        myJob::init();
        $records = myUser::getByWhere('system_role=' . _CUS_SYSTEM_ROLE_);
        ...
        $ret.= '  <div class="row tools-title">';
        $ret.= '     <span class="tbl-title">'.RTTCore::l('Customers','admin').'</span>';
