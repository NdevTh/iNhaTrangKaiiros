...
class Family extends Module
{
    public static function form($args)
    {
        ...
        if (Tools::getValue('id')){
            ...
            $record  = myBillOfMaterial::getById($list_id)[0];
        }
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            //echo $_POST["id_lang"];
            $_POST["date_upd"] = $currentDate;
            myBillOfMaterial::updateById(Tools::getValue('id'));
            $record = myBillOfMaterial::getById(Tools::getValue('id'))[0];
            //Tools::refresh();
        }
        $recordLang = array();
        ...
        $ret.= '          <div class="col-1">';
        $ret.= '             <label class="groupform">'.RTTCore::l('Description').'</label>';
        //$ret.= '          </div>';
        
        //$ret.= '          <div class="col-1">';
        $ret.= '             <textarea class="h-5" name="description" placeholder="Écrire ici"></textarea>';
        $ret.= '          </div>';
        $ret.= '       </div>';
        /* End 3rd Column */
        $ret.= '    </div>';
    ...
    public static function hookFamily($args)
    {
        global $actions;
        $table = 'bill_of_material';
        $records = array();
        myBillOfMaterial::init();
        $records = myBillOfMaterial::getRecords();
        $fieldsDisplay = array(
            'code'       => RTTCore::l('Code'),
            'title'      => RTTCore::l('Title'),
        ...
        $ret.= '  <div class="row tools-title">';
        $ret.= '     <span class="tbl-title">'.RTTCore::l('Family','admin').'</span>';
