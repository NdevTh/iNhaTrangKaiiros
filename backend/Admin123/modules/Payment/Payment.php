...
class Payment extends Module
{
    public static function form($args)
    {
        ...
        if (Tools::getValue('id')){
            ...
            $record  = (myPaymentMode::getById($list_id) !== null)?myPaymentMode::getById($list_id)[0]:array();
        }
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $list_id = myPaymentMode::save();
            $record  = myPaymentMode::getById($list_id)[0];
            $btnType = 'Edit';
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            */
            ...
            $_POST["date_upd"] = $currentDate;
            myPaymentMode::updateById(Tools::getValue('id'));
            $record = myPaymentMode::getById(Tools::getValue('id'))[0];
            //Tools::refresh();
        }
        ...
        /* Form Title */
        $ret.= '    <div class="row">';
        $ret.= '       <span class="frm-title">'.RTTCore::l('Payment Mode','admin').'</span>';
        $ret.= '    </div>';
        /* message Actions */
        if ($messages)
        {
    ...
    public static function hookPayment($args)
    {
