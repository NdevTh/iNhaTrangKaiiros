...
use classes\myPromo as myPromo;
...
class Promo extends Module
{
    public static function form($args)
    {
        ...
        if (Tools::getValue('id')){
            ...
            $record  = (myPromo::getById($list_id) !== null)?myPromo::getById($list_id)[0]:array();
        }
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $list_id = myPromo::save();
            $record  = myPromo::getById($list_id)[0];
            $btnType = 'Edit';
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            $_POST["date_upd"] = $currentDate;
            myPromo::updateById(Tools::getValue('id'));
            $record = myPromo::getById(Tools::getValue('id'))[0];
            //Tools::refresh();
        }
        ...
        $recordBom = array();
        if (array_key_exists('id_bom',$record) && $record['id_bom']){
        ...
        /* Form Title */
        $ret.= '    <div class="row">';
        $ret.= '       <span class="frm-title">'.RTTCore::l('Promotion','admin').'</span>';
        $ret.= '    </div>';
        /* message Actions */
        if ($messages)
        {
        ...
        $ret.= '           <div class="col-1">';
        $ret.= '               <div class="groupform">';
        $ret.= '                   <label>'.RTTCore::l('Image').'</label>';
        $ret.= '               </div>';
        $ret.= '               <div class="groupfield">';
        $ret.= '                   <input name="img_name" type="text" value="'.(!empty($record['img_name'])?$record["img_name"]:"").'"/>';
        $ret.= '               </div>';
        $ret.= '           </div>';
        ...
        $ret.= '                    <input class="select" onclick="showModal(\'modalBOM\')" id="txtbom" name="lang"  type="text" value="'.((array_key_exists('id_bom',$record) && !empty($recordBom['title']))?$recordBom["title"]:"").'"/>';
    ...
    public static function modalBOM()
    {
        ...
        $ret.= '  <div class="row tools-title">';
        $ret.= '     <span class="tbl-title">Liste</span>';
        $ret.= '     <span class="article">'.count($records).' Article(s)</span>';
        $ret.= '     <input id="inpSearch" onkeyup="myFunction(this,\''.$table.'\')" class="txt-search" type="text" placeholder="Introduir le mot clé pour rechercher" />';
        //$ret.= '     <span onclick="hideModal(\'myModal1\')" class="mdl-close">&times;</span>';
        $ret.= '  </div>';
        ...
        $ret.= '      </div>';
        $ret.= '      <div class="col-2 txt-right"';
        $ret.= '         <button onclick="hideModal(\'modalBOM\')"  class="btn-form" type="button">'.RTTCore::l('Cancel').'</button>';
        $ret.= '         <button onclick="selectValue(\'modalBOM\',\''.$table.'\',\'txtbom\');" class="btn-form" type="button">'.RTTCore::l('Ok').'</button>';
        $ret.= '      </div>';
        $ret.= '   </div>';
        $ret.= '</div>';
        /* End Field */
    ...
    public static function hookPromo($args)
    {
        global $actions,$status,$actives;
        $records = array();
        $table = "promo";
        myPromo::init();
        $records = myPromo::getRecords();
