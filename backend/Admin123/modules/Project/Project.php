...
class Project extends Module
{
    public static function form()
    {
        ...
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $_POST['id_status']           = _SUP_SYSTEM_ROLE_;
    ...
    public static function modalArticle()
    {
        ...
        if ($records)
        {
            ...
            foreach($records as $record)
            {
                $ret.= '<tr>';
                $indRow++;
                $ret.= '        <td class="txt-center"><input data-listId="'.(!empty($record['id_article'])?$record['article']:$indRow).'" onchange="changeChecked(\'id_article\',this.id)" id="company'.$indRow.'" type="checkbox" value="'.(!empty($record['title'])?$record['title']:$indRow).'"/></td>';
                foreach($fields as $field => $th)
                {
                    if ($field == 'title')
                    {
    ...
    public static function hookProject($args)
    {
        ...
        $records = myProject::getByWhere('`id_status`='._SUP_SYSTEM_ROLE_);
        $ret.= '<div class="row tools-title">';
        $ret.= '   <div class="col-2 txt-left">';
        //$ret.= '     <a class="btn" href="index.php?t='.Tools::getValue('t').'&pt='.Tools::getValue('pt').'&token='.Tools::getValue('token').'"><i class="bx bx-arrow-to-left"></i>'.RTTCore::l("Return","Admin").'</a>';
        $ret.= '   </div>';
