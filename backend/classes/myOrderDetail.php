...
class myOrderDetail extends ObjectModel
{
    ...
    {
        $sql = "SELECT ol.*, od.*, sum(od.order_quantity) as order_quantity,art.title FROM rtt_order_detail as od LEFT OUTER JOIN rtt_order_list as ol ON ol.id_order_list = od.id_order_list LEFT OUTER JOIN rtt_article as art ON art.id_article = od.id_article WHERE DATE(ol.date_add) = '".$currentDate."' AND ol.id_type < 2 GROUP BY id_article";
    ...
    {
        $sql = "SELECT art.*, sum(od.order_quantity) as order_quantity FROM rtt_article as art LEFT OUTER JOIN rtt_order_detail as od ON art.id_article = od.id_article GROUP BY id_article";
        //$sql = "SELECT art.*, sum(od.order_quantity) as order_quantity FROM rtt_article as art LEFT OUTER JOIN rtt_order_detail as od ON art.id_article = od.id_article WHERE op.date_add = ".date('Y-m-d H:i:s',time())." GROUP BY id_article";
        return Db::getInstance()->Execute($sql);
    }
