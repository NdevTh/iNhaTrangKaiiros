...
class WebCore
{
    ...
    public static function includeJS($arrJS = array()){
	    ...
	    public static function returnCSS($arrCSS = array()){
        //var_dump(self::$js);
        $ret = '';
        if (!empty($arrCSS))
        {
	    ...
	    public static function returnJS($arrJS = array()){
        //var_dump(self::$js);
        $ret = '';
    ...
    public static function autoSystemCheck($path="")
    {
        try{
        	...
        	if(!empty($path))
        	{
        		    $records = BundlesXMLFile::readFile('configs','url',$path);
        		    $listURL = BundlesXMLFile::getList('loc','configs',$path);
        	}else{
            ...
            //$listURL = BundlesXMLFile::getList('loc','configs');
            //var_dump($listURL);
            return self::autoXMLHistory($path);
        } catch(Error $e) {
        //} catch(Exception $e) {
            echo 'Error: ' . $e->getMessage();
        }
    ...
    public static function autoXMLHistory($path="")
    {
        try{
    ...
    public static function initTheme($type = "front")
    {
        /* create hooks data */
        ...
        foreach($hookData as $c => $record)
        {
            foreach($hookFields as $k => $field)
            {
        		...
        		if (strtolower($type) == "back")
        		{
        			   //BundlesXMLFile::writeFile('themes',$hookFields,"","../Bundles/xml");
    	     }else{
    	     	   BundlesXMLFile::writeFile('themes',$hookFields,"","../"._THEME_DIR_."/assets/xml");
    	     }
    ...
    public static function initFrontTab()
    {
        ...
        foreach($tabData as $c => $record)
        {
            ...
            BundlesXMLFile::writeFile('fronttabs',$tabFields,"",_THEME_DIR_."/assets/xml");
        }
        $listModule = BundlesXMLFile::getList('name','fronttabs',_THEME_DIR_."/assets/xml");
        //var_dump($listModule);
