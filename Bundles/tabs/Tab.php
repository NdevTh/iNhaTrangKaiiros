...
class Tab extends AdminTab
{
    public static function form()
    {
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            XMLFile::update('tabs',$whereClause,$updateFields,$query,"xml");
        }
        ...
        if (!empty(Tools::getValue('id')))
        {
            $whereClause = '[@id="'.Tools::getValue('id').'"]';
            $query = '//tabs/tab';
            $tabs = XMLFile::getXMLByWhere('tabs',$query,$whereClause,'xml');
            $record = $tabs[0];
            $btnType = 'Edit';
            //var_dump($record);
        }
    ...
    public static function viewTab()
    {
        $ret = '';
        //$ret.= 'view tab';
        if (Tools::getValue('act') == 'del'){
            ...
            $ret.= '</div>';
            XMLFile::deleteById('tabs',Tools::getValue('id'),'xml');
        }
        if (!file_exists('xml/tabs.xml'))
        {
            $ret.= 'Initialization Tab file...';
            Core::initTab();
        }
        
        $records = XMLFile::readFile('tabs','','xml');
