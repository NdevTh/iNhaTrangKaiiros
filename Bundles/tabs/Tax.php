...
class Tax extends AdminTab
{
    public static function form()
    {
        ...
        if (Tools::isSubmit('btnSubmitAdd')){
            ...
            $ret.= '</div>';
            //var_dump($_POST);
            XMLFile::writeFile('taxs',array(),"","xml");
        }
        ...
        if (Tools::isSubmit('btnSubmitEdit')){
            ...
            XMLFile::update('taxs',$whereClause,$updateFields,$query,"xml");
        }
        ...
        if (!empty(Tools::getValue('id')))
        {
            $whereClause = '[@id="'.Tools::getValue('id').'"]';
            $query = '//taxs/tax';
            $tabs = XMLFile::getXMLByWhere('taxs',$query,$whereClause,'xml');
            $record = $tabs[0];
            $btnType = 'Edit';
            //var_dump($record);
        }
        ...
        $ret.= '<form action="core.php?'.Tools::newUrl('t','tax').'" method="POST" class="form">';
        ...
        $ret.= '    <div class="row">';
        $ret.= '       <div class="groupform">';
        $ret.= '          <label>Rate</label>';
        $ret.= '       </div>';
        $ret.= '       <div class="groupfield">';
        $ret.= '          <input name="rate" type="text" value="'.(isset($record)?$record['rate']:"").'"/>';
        $ret.= '       </div>';
        $ret.= '    </div>';
        /*
        $ret.= '    <div class="row">';
    ...
    public static function viewTax()
    {
        $ret = '';
        //$ret.= 'view tab';
        if (Tools::getValue('act') == 'del'){
            ...
            $ret.= '</div>';
            XMLFile::deleteById('taxs',Tools::getValue('id'),'xml');
        }
        if (!file_exists('xml/taxs.xml'))
        {
            $ret.= 'Initialization Tab file...';
            Core::initTax();
        }
        
        $records = XMLFile::readFile('taxs','','xml');
        //$folders = Tools::scanFolder('taxs');
        //var_dump($folders);
        //var_dump($records);
        if (!empty($records))
        {
            $fields = XMLFile::getFields('taxs','xml');
            $ret.= '<div class="row">';
